...
A tiny templating engine with **time-travel built into the primitive**, deliberately auditable, and CSP-safe.
...
- `data-each` without `data-key` rebuilds the list on any change. With a key, only re-indexed items rebuild.
