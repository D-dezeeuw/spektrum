...
| `data-each ... data-key="expr"` | Keyed reconciliation. Items keyed identically keep their DOM, listeners, focus, and uncommitted input state across reorder (the same clone is reused; bindings re-subscribe to the new index). A row is re-bound when its index changes **or** when the object at that key is replaced by a different one, so the immutable-update idiom (`items.map(r => ({...r, …}))`) re-renders correctly while carried-over rows stay untouched. Without a key, the no-key path appends/pops the tail and rebuilds on interior changes. |
| Scope variables inside `data-each` | The loop variable (`item` or whatever `data-as` declares), plus `$index`, `$first`, `$last`, `$path`. `$path` is the row's full state path as a string (e.g. `items.3`) — usable in text (`{{$path}}`) and readable from a handler's `scope` argument. For row-relative *targets*, write scope-resolved paths instead: `data-id="item.completed"` / `data-model="item.completed"` (`{{…}}` never runs inside attribute values). `data-stable-key` is accepted as a no-op for back-compat — every keyed row reuses across reorder by default. |
...
Built-in `data-fn` handlers: `setValue`, `addValue` (plus `trigger`, its deprecated alias), `setText`, `setStyle`, `toggle`. Register your own with `defineFn(name, handler)`. Handler signature: `(el, state, delta, value, event?, scope?)`. The `event` argument is the DOM `Event` for `data-action="click"`-style bindings, or `undefined` for `data-action="cycle"` (subscription-driven, no event in scope). The trailing `scope` is the `data-each` iteration scope (`undefined` outside a loop): built-ins resolve `data-id="item.field"` through it to the row's real state path, and custom handlers can read `scope.item` (the row object), `scope.$index`, or `scope.$path` directly.
