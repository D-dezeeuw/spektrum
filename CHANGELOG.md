...
- **Example: the basket's remove button works again.** Same root cause as the fork-restore fix below: `removeAt` parsed a row index out of a `data-id` the pre-1.0 text-rewriter used to rewrite per row, so under the current engine it computed `Number('item')` → `NaN` and the filter removed nothing. It now identifies the clicked row from the iteration scope (`scope.item`) and filters by identity — correct under any filtering or reordering.
- **Example: the fork "restore" button works again.** `restoreFork` read a `data-id` that the pre-1.0 text-rewriter used to rewrite per row; the 1.0 scope refactor removed that mechanism, so the handler computed `Number('f')` → `NaN` and silently no-opped. It now reads the row index from the iteration scope (`scope.$index`) — the mechanism the engine actually provides — and re-applies `add` entries via `addValue`.

### Documentation

*A progressive-disclosure pass for AI discovery. The doc surface is now explicitly layered — small entry points that orient and route, references that carry the depth — and the guide layers that still taught the pre-1.0 engine are re-synced to the shipped surface.*

- New root **[`llms.txt`](llms.txt)** — one-page discovery map with the "check your priors" list (how Spektrum differs from Vue/React/Alpine) up front. Published at the docs-site root by the Pages workflow and shipped in the npm package.
- New root **[`CLAUDE.md`](CLAUDE.md)** — contributor orientation: source-of-truth order (source > docs > guides; disagreement means the stale layer gets fixed in the same change), the hard gates, commands, and a read-this-when table.
- **`.claude/skills/spektrum/SKILL.md` rewritten against the current engine.** The skill still taught the pre-1.0 surface: `trigger` as the additive primitive (now the deprecated alias of `addValue`), the removed regex `rewriteScope` mechanism and its `data-as` warnings, `data-stable-key` as an opt-in (now a no-op — keyed rows always reuse DOM), and no mention of the `$index` / `$first` / `$last` / `$path` scope variables. Also removed a debugging claim the engine contradicts (a system subscribed to `user.name` **does** fire on `setValue('user', {name})` — the path resolves in the delta). Duplicated reference dumps are slimmed into summaries that point at `docs/`, so there is less surface to go stale again.
- **`AGENTS.md`** — new "Check your priors — Spektrum is not Vue/React/Alpine" table up front; mutator examples modernized to `addValue`.
- **The npm package now ships `llms.txt`, `AGENTS.md`, `CHANGELOG.md`, and `docs/`** alongside the code, so agents working inside `node_modules/spektrum` see the same layers as the repository instead of just README + source. `README.md` gains a "For AI agents" section describing the layering.
- **Drift fixes across `docs/`:** [trade-offs](docs/trade-offs.md) described `computed` as writing the delta only with mid-tick reads seeing stale values — the engine writes both state and delta precisely so mid-tick reads are fresh; [bindings](docs/bindings.md) suggested `data-id="{{$path}}.completed"`, but `{{…}}` never runs in attribute values — the working idiom is scope-resolved `data-id="item.completed"` — and its built-ins list gains `addValue` plus the trailing `scope?` handler argument; [constraints](docs/constraints.md) quoted the clawed-back 13,952 B engine cap (the enforced cap is 13,696 B raw / 6,240 B gz) and now defers to `scripts/size.js` as authoritative; [time-travel](docs/time-travel.md)'s fork-restore recipe and [api](docs/api.md)'s built-ins list modernized to `addValue`; size/line counts unified to the measured build (~1,430 lines, ~13 kB min / ~6 kB gz) across README, SECURITY, philosophy, constraints, and docs/README.
- **`CONTRIBUTING.md`** — the post-refactor drift ritual now greps every doc layer (`docs/`, `AGENTS.md`, the skill, `llms.txt`, `README.md`, `example/`), not just `docs/`; the skill and AGENTS.md staying stale for months is exactly the gap it closes.
- **Example** teaches current idioms: counter buttons use `data-fn="addValue"` (the deprecated `data-fn="trigger"` spelling remains supported engine-side).
- **The example page is now a self-documenting feature tour.** Below the counter/basket flagships, a third isolated instance powers ten sections — state/delta/tick (with the `undefined`-write footgun live), text & attribute bindings, `data-model` modifiers, `data-if`, `data-each` (keyed reuse, `$index`/`$first`/`$last`/`$path`, the `<template>` form inside a `<table>`, nesting), `data-action` modifiers + `cycle` + `data-ref`, `computed`/`addAsync`/`refresh`, time-travel (checkpoint/jump/undo with live cursor readouts via `onRecord`), the agent surface (`describe`/`explain`/`findByIntent` with element flashing/`attempt` commit-discard), and the companion catalog. Each section pairs the live demo with **its own markup, extracted from the DOM before `bindDOM()` runs**, and the wiring JS via `fn.toString()` — so the code shown is by construction the code that ran.
...
- **`spektrum/persist`: `maxEntries` keeps the newest entries.** The cap sliced from the front, keeping the **oldest** — so an over-cap restore silently booted the app into ancient state instead of where the user left it. Now matches the direction the engine's own `historyLimit` trims. *(A companion page-hide flush for debounced saves was prototyped and dropped — a `visibilitychange` listener with teardown exceeds the module's size budget. The `debounce` docs point to the manual `saveHistory`-on-`visibilitychange` pattern instead.)*
...
- **Snapshot cloning now uses `structuredClone`.** For the supported state shape (plain objects, arrays, primitives) this is identical to the previous hand-rolled walker. The one difference: a **function or symbol placed in state** — already unsupported, and already broken under `serialize()` and replay — now makes a `snapshotEvery` capture *throw* rather than silently copying the reference. Fail-loud on unsupported state is the intended trade.

### Internal

- **No size caps were raised.** Every fix in this batch ships **under the pre-existing budgets**; the caps in [`scripts/size.js`](scripts/size.js) are unchanged. Where new logic didn't fit, it was trimmed until it did — or, where it still didn't, dropped (the persist page-hide flush). The reclaims:
  - Engine: the hand-rolled `deepClone` became the `structuredClone` platform global (equivalent on every reachable input — state is plain objects/arrays/primitives, `SAFE_KEY` keeps `__proto__` out, NaN/Infinity survive), and a now-dead `cacheSet` guard was removed. The CSP + binding-containment + keyed-`data-each` fixes ship inside the original 13,696 B raw / 6,240 B gz.
  - `spektrum/mcp`: the four guard-hardening fixes cost ~1.1 KB raw of new logic, fully absorbed by trimming pre-existing fat — a schema-builder (`OBJ`) plus shared field consts dedupe the repeated JSON-Schema boilerplate, `structuredClone` replaces the clone helper, validation messages route through one `bad(field, type)` builder, and verbose tool descriptions were tightened. Net result is **smaller than before** (raw 4,656 vs 5,320 pre-session), under the unchanged 5,376 B / 2,112 B caps.
  - `spektrum/compile` emitter rewrite costs zero runtime bytes (build-time only).
